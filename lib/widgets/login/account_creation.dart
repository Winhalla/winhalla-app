    ...
    if (widget.steamLoginUri != null) {
      Future.delayed(const Duration(milliseconds: 0), () async {
        try {
          ...
          String? steamId = Uri.tryParse(openId.data["openid.claimed_id"] as String)?.pathSegments.last;
          if (steamId == null) {
          ...
          showInfoDropdown(context, kRed, "Error:",
              body: Text(
                ...
                style: Theme.of(context).textTheme.bodyText2?.merge(InheritedTextStyle.of(context).kBodyText4),
              ),
              fontSize: 25,
              column: true,
              isError: true);
              ...
              Align(
                ...
                child: GestureDetector(
                  ...
                  child: Row(
                    ...
                    children: [
                      Transform.rotate(
                          angle: 180 * pi / 180, child: Icon(Icons.arrow_forward, color: kRed.withOpacity(0.8))),
                      const SizedBox(
                        width: 7,
                      ),
                      Text(
                        "Back",
                        style: InheritedTextStyle.of(context).kBodyText2.apply(color: kRed.withOpacity(0.8)),
                      )
                    ],
                  ),
                ),
                          ...
                          onTap: () {
                            ...
                            setState(() {
                              listKey.currentState?.removeItem(index,
                                  (_, animation) => animatedFakeContainer(context, index, animation, name, fileName),
                                  duration: const Duration(milliseconds: 150));
                              items.add({
                                "platformId": accounts[index]["platformId"],
                                "name": fileToName(accounts[index]["platformId"])
                              });
            ...
            if (accounts.length < 3)
              GestureDetector(
                ...
                onTap: () async {
                  ...
                  var result = await showDialog(context: context, builder: (context) => PopupWidget(context, items));
                  if (result != null) {
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      ...
                      style: InheritedTextStyle.of(context).kBodyText3.apply(color: kRed),
                    ))
                  ],
                ),
              ),
            Row(
              mainAxisAlignment: alreadyCreatedAccount ? MainAxisAlignment.spaceBetween : MainAxisAlignment.end,
              children: [
                if (alreadyCreatedAccount)
                  GestureDetector(
                    onTap: () {
                ...
                GestureDetector(
                  ...
                  onTap: () async {
                    ...
                    String? referral = await secureStorage.read(key: "sponsorshipReferral");
                    ...
                    var accountData = await callApi.post(
                        alreadyCreatedAccount
                            ...
                            : '/auth/createAccount?a=b' + (link == "no data" ? "" : '&linkId=$link') + (referral == null ? "" : "&referral=$referral"),
                        jsonEncode(
                          {"accounts": accounts},
                        ),
                        showError: false);
...
Widget animatedFakeContainer(item, int index, Animation<double> animation, String name, String file) => FadeTransition(
      opacity: Tween(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: animation, curve: Curves.easeOut)),
